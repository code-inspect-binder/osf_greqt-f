...
tiff("Figure2.tif",width=12,height=20,units="cm",res=600,compression="lzw")

#Here starts the plotting
par(mar=c(3.5,11,3.5,3),mgp=c(2,0.8,0))
...
text(17.5,par("usr")[3]+par("usr")[4]/2,expression(paste("Relative similarity (t value)")),srt=270,xpd=T,font=2)
...
for(i in 1:length(list.res.all)){
  ...
  rect(0:14,ytop[i]+gap/2,1:15,ybot[i]-gap/2,col=alpha(percols,abs(result$p.compare-0.5)*2),border=NA)
...
for(i in 1:length(list.res.all)){
  lines(0:16-0.5,ad(list.res.all[[i]]$DELTA+ymid[i]),col="#000000")
}
...
lseq<-seq(0,1,l=101)
rect(c(-0.5,15),top+ran*0.02,c(0,15.5),top+ran*0.05,col=percol(2),xpd=T,border=NA)
rect(rseq[1:100],top+ran*0.02,rseq[2:101],top+ran*0.05,xpd=T,col=alpha(percol(101),abs(lseq-0.5)*2),border=NA)
text(seq(0,15,l=5),top+ran*0.035,seq(0,1,l=5),cex=0.8,xpd=T)
text(-1,top+ran*0.07,"Sensitive period likelihood",pos=4,xpd=T,font=2)


rctas<-c(rctas,list(max(unlist(rctas))+c(1:3)))

rect(0,ytop[sapply(rctas,function(x){x[1]})]+gap/2,15,ybot[sapply(rctas,function(x){x[length(x)]})]-gap/2,xpd=NA)

legend("bottomright",c("+2","−2"),pch=21,col=c("#008800","#888800"),pt.bg=c("#55FF55","#FFFF55"),cex=1,xpd=NA,inset=c(-0.30,-0.1),bty="n")
